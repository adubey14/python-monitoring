# Motherboard Temperature Measurement Using IPMI

class Sensors_Temp_Motherboard_Scalar(Sensor):
      def __init__(self):
          # Configuration parameters common to all sensors
          self.sensor='Temp'
          self.device='Motherboard'
          ...
          if 'pion' not in self.nodename:
             ...
             self.IPMIDevQueryString=['Serverboard temp']
      ...
      def senseandlog(self,timestamp):
          ...
          for ipmiquery in self.IPMIDevQueryString:
              ...
              try:
                  ...
                  self.status.setProperty('LastReadReading',str(round(reading,3)))
              except:
                     print "error in running ipmi command in Sensors_Voltage_Motherboard_Scalar"
                     print "The command given was %s"%IPMICommandTemplate.substitute(sdrname=self.sdrname,querystring=ipmiquery)
                     traceback.print_exc(file=sys.stdout)
                     print sys.exc_info()
                     self.status.setProperty('LastReadReading','error in running ipmi command'+str(sys.exc_info()))
                     continue
              if (self.val[ipmiquery]> reading) and (self.val[ipmiquery]- reading >self.downdeadband):
                   ...
                   self.status.setProperty('LastReportedReading',str(round(reading,3)))
                   log(output)
                   continue
...
sensors['Sensors_Temp_Motherboard_Scalar']= Sensors_Temp_Motherboard_Scalar()
